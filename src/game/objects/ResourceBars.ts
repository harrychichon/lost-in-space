...
export function drawResourceBars(scene: Scene, state: ReturnType<typeof GameState.get>) {
    ...
    container.add(scene.add.text(panelX + s(74), panelY + s(34), 'RESOURCES', {
    ...
    resources.forEach((res, i) => {
        ...
        container.add(drawResourceIcon(scene, res.type, iconX, rowTop + s(20), res.color) as Phaser.GameObjects.GameObject[]);

        container.add(scene.add.text(labelX, rowTop, res.key, {
            fontFamily: titleFont,
            fontSize: px(37),
            color: '#a7aeb8',
        }).setScale(0.5, 0.5));

        container.add(scene.add.rectangle(barLeft + barW / 2, barY, barW, barH, 0x2a2f38).setOrigin(0.5));

        const fillW = Math.max(0, Math.min(100, res.value)) / 100 * barW;
        container.add(scene.add.rectangle(barLeft + fillW / 2, barY, fillW, barH - s(2), res.color).setOrigin(0.5));

        container.add(scene.add.text(percentX, rowTop + s(19), `${Math.round(res.value)}%`, {
            fontFamily: titleFont,
            fontSize: px(46),
            color: res.accent,
...
function drawResourceIcon(
    ...
    if (type === 'oxygen') {
        ...
        objs.push(scene.add.text(x + s(10), y + s(8), '2', {
