...
export function drawResourceBars(scene: Scene, state: ReturnType<typeof GameState.get>) {
    ...
    container.add(
        scene.add
            .text(panelX + s(74), panelY + s(34), 'RESOURCES', {
    ...
    resources.forEach((res, i) => {
        ...
        container.add(
            scene.add
                .rectangle(barLeft + fillW / 2, barY, fillW, barH - s(2), res.color)
                .setOrigin(0.5)
        )

        container.add(
            scene.add
                .text(percentX, rowTop + s(19), `${Math.round(res.value)}%`, {
                    fontFamily: titleFont,
                    fontSize: px(46),
                    color: res.accent,
                })
