...
export class CompanionEvent extends Scene {
    ...
    create() {
        ...
        const ship = this.add.image(width * 0.22 - 120, height * 0.66 + 110, 'ship_default').setOrigin(0.5, 1);
        ship.displayHeight = 256;
        ship.scaleX = ship.scaleY;

        // Dog — small figure near the wreck
        const dogX = width * 0.55 - 30;
        const dogGroundY = height * 0.66 + 80;
        createDogSprite(this, dogX, dogGroundY, { flipX: true });
